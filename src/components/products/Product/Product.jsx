...
const Product = () => {
  ...
  return !product ? null : String(id) !== String(product?.id) ? (
    "Loading"
  ) : (
    <>
      <Navbar />
      <div className="product">
        ...
        <p className="product__price">{Math.round(retail_price_brutt * 100) / 100} zł</p>
        <button className="product__cart-button" onClick={() => handleAddToCart(id)}>Add to Cart</button>
        {/* <p className="product__stock">{stock_status}</p> */}
        <div className="product__img">
          <img src={image_url}></img>
        </div>
        <div className="product__wrapper">
          <button className="product__fav-button" onClick={() => handleFavToggle(id)}>
